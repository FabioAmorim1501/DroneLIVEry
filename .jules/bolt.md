...
## 2024-06-10 - Avoiding O(N²) math calculations in nested algorithm loops
**Learning:** In Nearest Neighbor or similar algorithms, calculating complex arithmetic operations (like trigonometric functions used in the Haversine formula for distance) inside nested loops causes a severe O(N^2) bottleneck. When computing values against a fixed reference point (e.g., return distance to a base Hub from each destination candidate), these redundant recalculations dramatically degrade performance.
**Action:** Always extract invariant complex calculations from inner loops. Pre-calculate the result (e.g., distances from all candidates to the hub) in an O(N) pre-processing loop and cache it in an O(1) Hash Map (`TDictionary<TKey, TValue>`). Inside the complex main loop, substitute the mathematical calculation with a lightweight `TryGetValue` lookup.
