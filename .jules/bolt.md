...
## 2024-06-11 - Haversine Math Optimization inside Nested Loops
**Learning:** In the Nearest Neighbor algorithm, evaluating return-to-base constraints requires computing the geographic distance between the candidate destination and the central hub using the Haversine trigonometric formula. Because neither the order's destination nor the hub location changes during the iteration, placing this computationally expensive math inside the O(N²) `while LPedidosPendentes.Count > 0 do` loop introduces severe, redundant processing overhead as the queue size grows.
**Action:** When a computationally expensive operation (like trigonometric math) relies on parameters that remain constant throughout an algorithm's execution, pre-calculate the result in an initial processing loop and cache it in a `TDictionary`. Replace the mathematical operation in the main algorithm loop with an O(1) `TryGetValue` lookup.
